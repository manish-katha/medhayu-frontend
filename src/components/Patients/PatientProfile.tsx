...
import { Calendar, DollarSign, Heart, Plus, Edit, Stethoscope } from 'lucide-react';
import { format, formatDistanceToNow } from 'date-fns';
import { getPatientProfile } from '@/services/patient.service';
import StatCard from './Profile/StatCard';
import VisitsTab from './Profile/VisitsTab';
import BillingTab from './Profile/BillingTab';
import ReportsTab from './Profile/ReportsTab';
import CaseStudiesTab from './Profile/CaseStudiesTab';
...
const PatientProfile: React.FC<PatientProfileProps> = ({ patientId }) => {
  ...
  const memberSince = patient.lastVisit ? new Date(patient.lastVisit) : new Date(); // Fallback
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row items-start gap-6">
            ...
            <div className="flex-grow">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-muted-foreground mt-2">
                <span>ID: {patient.cin || patient.oid}</span>
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="overview">
          <OverviewTab visits={visits} invoices={invoices} />
        </TabsContent>
        ...
        <TabsContent value="billing">
         ...
         <TabsContent value="reports">
          <ReportsTab reports={labReports} />
        </TabsContent>
         <TabsContent value="cases">
          <CaseStudiesTab caseStudies={caseStudies} />
        </TabsContent>
      </Tabs>
    </div>
  );
