...
import { BookActions } from '../medhayu/Article renderer/book-actions';
    ...
    return (
        <div className="relative">
            ...
            <Card className={cn("group flex flex-col overflow-hidden h-full", isSelected && "ring-2 ring-primary")}>
                <Link href={`/medhayu/books/${book.id}`} className="block overflow-hidden">
                ...
                <CardFooter className="p-3 flex-col items-start bg-background/90 z-10 mt-auto">
                    <div className="w-full flex justify-between items-start">
                        <Link href={`/medhayu/books/${book.id}`} className="flex-1 pr-2">
...
function BookListItem({ book }: { book: Book }) {
    return (
        <div className="flex items-center justify-between p-2 rounded-md hover:bg-muted/50">
            <Link href={`/medhayu/books/${book.id}`} className="flex items-center gap-4 flex-1 min-w-0">
  ...
  return (
    <>
      ...
      <Collapsible open={isOpen} onOpenChange={setIsOpen} asChild>
        <Card className="group overflow-hidden flex flex-col h-full transition-shadow hover:shadow-xl bg-card/50">
          <CollapsibleTrigger asChild>
              <div className='cursor-pointer flex flex-col flex-grow'>
                  ...
                  <CardContent className="flex-1 flex flex-col justify-center items-center relative py-8">
                    <div className="relative h-48 w-32 group-hover:scale-105 transition-transform">
                        {series.volumes.slice(0, 3).map((volume, index) => (
                            <div 
                                key={volume.id}
                                className="absolute w-full h-full rounded-md shadow-lg border-2 border-background/50"
                                style={{
                                    zIndex: 3 - index,
                                    transform: `translateX(${index * 4}px) translateY(-${index * 4}px) rotateZ(${index * 2}deg)`,
                                    top: 0,
                                    left: 0,
                                }}
                            >
                                <Image
                                    src={volume.profileUrl || 'https://placehold.co/400x600.png'}
                                    alt={`${series.seriesName} cover ${index + 1}`}
                                    fill
                     ...
                     <div className="relative mt-6 group/desc">
                        <p className="text-sm text-center text-muted-foreground line-clamp-2 max-w-xs">
                          {series.description || 'No description available for this series.'}
                        </p>
                        <Button variant="outline" size="icon" className="absolute -top-1 -right-8 h-7 w-7 opacity-0 group-hover/desc:opacity-100 transition-opacity" onClick={(e) => { e.stopPropagation(); setIsDescriptionDialogOpen(true); }}>
...
export function BookBrowser({ groupedBooks }: { groupedBooks: Record<string, Book[]> }) {
    const [view, setView] = useState<'grid' | 'list'>('grid');
    const [activeTab, setActiveTab] = useState('series');
    
    const allBooks = useMemo(() => Object.values(groupedBooks).flat(), [groupedBooks]);
    
    const allSeries = useMemo(() => {
        const seriesMap: { [seriesName: string]: SeriesGroup } = {};
        allBooks.forEach(book => {
            if (book.volumeInfo) {
                if (!seriesMap[book.volumeInfo.seriesName]) {
                    seriesMap[book.volumeInfo.seriesName] = { seriesName: book.volumeInfo.seriesName, description: '', volumes: [] };
                }
                seriesMap[book.volumeInfo.seriesName].volumes.push(book);
            }
        });
        
        Object.values(seriesMap).forEach(s => {
            s.volumes.sort((a, b) => (a.volumeInfo?.volumeNumber || 0) - (b.volumeInfo?.volumeNumber || 0));
            // A series description is the shortDescription of its first volume.
            if (s.volumes[0] && s.volumes[0].shortDescription) {
    ...
    const allStandalone = useMemo(() => allBooks.filter(book => !book.volumeInfo), [allBooks]);
