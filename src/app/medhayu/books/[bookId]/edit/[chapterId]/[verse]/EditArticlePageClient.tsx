...
export function EditArticlePageClient(props: { initialArticleData: { book: BookContent, chapter: any, article: ArticleType }, params: { bookId: string; chapterId: string; verse: string; } }) {
    ...
    useEffect(() => {
        ...
        setIsFullyLoaded(true); // Set loaded after initial render with data
        return () => {
    ...
    useEffect(() => {
        if (state?.success && isFullyLoaded) { // Check isFullyLoaded here as well
            toast({
              title: (
    ...
    const addBlock = useCallback((type: string) => {
        ...
        // After adding, focus the new editor instance
        setTimeout(() => {
    ...
    const handleHighlight = (id: string, isMark?: boolean, markIndex?: number) => {
        setHighlightTarget(id);
        
        const contentArea = document.querySelector('.article-editor-content');
        if (!contentArea) return;
        
        const element = contentArea.querySelector(isMark ? `span[data-mark-id='${id}']` : `[id='${id}']`);
