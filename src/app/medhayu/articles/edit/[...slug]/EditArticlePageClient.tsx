...
import React, { useActionState, useState, useEffect, useCallback, useTransition, useRef } from 'react';
...
export function EditArticlePageClient(props: { initialArticleData: { book: BookContent, chapter: any, article: ArticleType }, params: { bookId: string; slug: string[]; } }) {
    ...
    const editorInstances = useRef(new Map<string, Editor>());
    ...
    const addBlock = useCallback((type: string) => {
        ...
        setTimeout(() => {
          const editorInstance = editorInstances.current.get(newBlock.id!);
    ...
    const removeEditorInstance = useCallback((id: string) => {
        const editorToDestroy = editorInstances.current.get(id);
    ...
    const handleQuoteCreated = React.useCallback((newQuote?: Quote) => {
        ...
        if (!newQuote || !range || !editorInstances.current.size) {
             setQuoteDialogState({ open: false, text: '', range: null });
            return;
        }
        
        const editor = activeEditorInstance;
        if (!editor) return;
        ...
        setQuoteDialogState({ open: false, text: '', range: null });
    }, [activeEditorInstance, quoteDialogState.range]);
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                    <Sheet>
                        ...
                        <SheetContent className="w-full sm:max-w-4xl">
                            ...
                            <div className="p-4 note-editor prose-styling">
                                        ...
                                        blocks.map((block, index) => {
                                            ...
                                            return (
                                                <React.Fragment key={block!.id}>
                                                    ...
                                                    <div className={cn("article-editor-block relative group", highlightTarget === block!.id && 'highlight-flash')} id={block!.id}>
                                                        ...
                                                        <RichTextEditor
                                                            ...
                                                            onFocus={() => setActiveEditorInstance(editorInstances.current.get(block!.id!) || null)}
