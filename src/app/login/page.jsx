...
import { login, registerUser } from '@/actions/signin.action';
...
// ------------------ Multi-step Signup Steps ------------------
const Step1 = ({ data, setData }) => (
  <div className="w-full max-w-md mx-auto space-y-3">
    <div className="grid grid-cols-2 gap-3">
      <div className="input-field">
        <FontAwesomeIcon icon={faUser} className="input-icon" />
        <Input placeholder="First Name" value={data.firstname} onChange={e => setData({ ...data, firstname: e.target.value })} required />
      </div>
      <div className="input-field">
        <FontAwesomeIcon icon={faUser} className="input-icon" />
        <Input placeholder="Last Name" value={data.lastname} onChange={e => setData({ ...data, lastname: e.target.value })} required />
      </div>
    </div>
    <div className="input-field">
      <FontAwesomeIcon icon={faPhone} className="input-icon" />
      <Input placeholder="Phone Number" value={data.phone} onChange={e => setData({ ...data, phone: e.target.value })} required />
    </div>
    <div className="input-field">
 ...
 <RadioGroup
  ...
  onValueChange={value =>
    setData({
      ...
      isPracticener: value === 'practitioner',
    })
  }
  className="flex justify-around pt-2"
>
  <Label className="flex items-center gap-2 cursor-pointer">
    <RadioGroupItem value="student" />I am a Student
  </Label>
  <Label className="flex items-center gap-2 cursor-pointer">
    <RadioGroupItem value="doctor" />I am a Doctor
  </Label>
  <Label className="flex items-center gap-2 cursor-pointer">
    <RadioGroupItem value="practitioner" />I am a Practitioner
  </Label>
</RadioGroup>
  </div>
);
...
const Step2 = ({ data, setData }) => (
  <div className="w-full max-w-md mx-auto space-y-3">
    <div className="flex items-center space-x-2">
      <Checkbox checked={data.isBams} onCheckedChange={checked => setData({ ...data, isBams: checked })} />
      <Label>BAMS</Label>
    </div>
    {data.isBams && (
      <div className="pl-6 space-y-3">
        ...
        <div className="input-field">
          ...
          <Input placeholder="UG College Name" value={data.ugCLgName} onChange={e => setData({ ...data, ugCLgName: e.target.value })} />
        </div>
      </div>
    )}
  </div>
...
const Step3 = ({ data, setData }) => (
  <div className="w-full max-w-md mx-auto space-y-3">
    {data.professionalRole === 'student' ? (
      <div className="input-field">
        <Textarea placeholder="Your Future Plans..." value={data.futurePlan} onChange={e => setData({ ...data, futurePlan: e.target.value })} />
      </div>
    ) : (
      <div className="input-field">
        <Input placeholder="Practice Center Name" value={data.practiceCenterName} onChange={e => setData({ ...data, practiceCenterName: e.target.value })} />
      </div>
    )}
  </div>
);
...
const Step4 = ({ data, setData }) => (
  <div className="w-full max-w-md mx-auto space-y-3">
    ...
    <div className="input-field">
      <FontAwesomeIcon icon={faKey} className="input-icon" />
      <Input type="password" placeholder="Confirm Password" value={data.confirmPassword} onChange={e => setData({ ...data, confirmPassword: e.target.value })} required />
    </div>
  </div>
);
...
const MultiStepSignUpForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Role selection
    professionalRole: 'student', isStudent: false, isAuthor: false, isSuperadmin: false,
    isBams: false, isMdMs: false, isDoctor: false, isSpecialist: false, isPracticener: false,
    // Education / Work
    ugCLgName: '', ugPincode: '', Batch: '', pgClgName: '', pgPinCode: '', specialization: '', workType: '', practiceCenterName: '',
    // Additional
    futurePlan: '', registerationNo: '', multiclinic: false,
  });
    ...
    <form onSubmit={handleFormSubmit} className="sign-up-form">
      ...
      <div className="flex justify-between mt-4">
        ...
        {currentStep < 3 ? (
          ...
          <SubmitButton label="Finish" pending={pending} />
        )}
      </div>
    </form>
  );
    ...
    <div className={cn("login-container", isSignUpMode && "sign-up-mode")}>
      <div className="forms-container">
        <div className="signin-signup">
          ...
          {isSignUpMode && <MultiStepSignUpForm />}
        </div>
      </div>
